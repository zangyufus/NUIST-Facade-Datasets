...
def collect_point_label(anno_path, out_filename, file_format='txt'):
  ...
  #glob.glob()方法返回所有匹配的文件路径名
  #os.path.basename(path)返回对应的文件名
 
  for file in glob.glob(os.path.join(anno_path, '*.txt')):#dgcnn/data/Facade_raw_data/Area_1/facade_1/Annotations/wall_1.txt
      cls = os.path.basename(file).split('_')[0]   #  对指定路径拆分后返回尾部
      print(file)
